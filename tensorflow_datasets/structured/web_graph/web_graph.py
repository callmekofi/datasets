...
truth (dest outlinks), only present in test split.
"""

_FORMAT_VERSIONS = [
    'sparse', 'dense', 'de-sparse', 'de-dense', 'in-sparse', 'in-dense'
]
...
class WebGraph(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager,
    ...
    web_graph_homepage = 'https://storage.googleapis.com/gresearch/web_graph/'
    subfolder = web_graph_homepage + self.builder_config.subfolder
    shards = self.builder_config.num_shards

    train_files = [
        os.path.join(subfolder, f'train.tfr-{i:05}-of-{int(shards):05}')
        for i in range(shards)
    ]
    path = dl_manager.download_and_extract({'train': train_files})
    pipeline_train = pipeline | 'train' >> beam.Create(path['train'])

    train_t_files = [
        os.path.join(subfolder,
                     f'train.transpose.tfr-{i:05}-of-{int(shards):05}')
        for i in range(shards)
    ]
    path = dl_manager.download_and_extract({'train_t': train_t_files})
    pipeline_train_t = pipeline | 'train_t' >> beam.Create(path['train_t'])

    test_files = [
        os.path.join(subfolder, f'test.tfr-{i:05}-of-{int(shards):05}')
        for i in range(shards)
    ]
    path = dl_manager.download_and_extract({'test': test_files})
    pipeline_test = pipeline | 'test' >> beam.Create(path['test'])
    return {
        'train': self._generate_examples(pipeline_train, split='train'),
        'train_t': self._generate_examples(pipeline_train_t, split='train_t'),
        'test': self._generate_examples(pipeline_test, split='test')
    }

  def _generate_examples(self, pipeline, split: str):
    """Yields examples."""
    beam = tfds.core.lazy_imports.apache_beam
    ...
    def _process_example(example: tf.train.Example, is_test=False):
      example = tf.train.Example.FromString(example)
      row_tag = _get_int_feature(example, 'row_tag')[0]
      col_tag = _get_int_feature(example, 'col_tag')

      return_dict = {'row_tag': row_tag, 'col_tag': col_tag}

      if is_test:
        gt_tag = _get_int_feature(example, 'gt_tag')
        return_dict['gt_tag'] = gt_tag
